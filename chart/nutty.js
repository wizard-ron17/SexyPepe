...
async function fetchData() {
    try {
        const response = await fetch('https://rons-server.netlify.app/.netlify/functions/server/api/historical-trades/nutty');
...
function createChart(data, interval) {
    ...
    const options = {
        ...
        title: {
            text: `$NUTTY/ALPH (${interval === 60 ? '1H' : interval === 240 ? '4H' : interval === 720 ? '12H' : interval === 10080 ? '1W' : '1D'})`,
            align: 'left',
            style: {
                fontSize: '16px',
                fontWeight: '600',
